...
if [ -d "/var/lib/mysql/$dbname" ]; then
	echo "La base de datos roundcube ya existe"
	echo "La base de datos roundcube ya existe" >>/script/registro.txt
else 	
	echo "La base de datos roundcube no existe"
	echo "Iniciando la creación de la base de datos roundcube"
	echo "La base de datos roundcube no existe" >>/script/registro.txt
	echo "Iniciando la creación de la base de datos roundcube" >>/script/registro.txt
...
#Descarga del servidor RoundCube
wget https://github.com/roundcube/roundcubemail/releases/download/1.6.1/roundcubemail-1.6.1-complete.tar.gz >/dev/null 2>&1
if [ $? -eq 0 ]; then
	echo -e "${Verde}Descarga exitosa del archivo RoundCube${Defecto}"
	echo "Descarga exitosa del archivo RoundCube" >>/script/registro.txt
else
	echo -e "${Rojo}Error al descargar el archivo RoundCube${Defecto}"
	echo "Error al descargar el archivo RoundCube" >>/script/registro.txt
fi

#Descomprimir el archivo RoundCube
tar xvzf roundcubemail-1.6.1-complete.tar.gz >/dev/null 2>&1
if [ $? -eq 0 ]; then
	echo -e "${Verde}La extracción del archivo RoundCube fue exitosa${Defecto}"
	echo "La extracción del archivo RoundCube fue exitosa" >>/script/registro.txt
else
	echo -e "${Rojo}Error en la extracción del archivo RoundCube${Defecto}"
	echo "Error en la extracción del archivo RoundCube" >>/script/registro.txt
fi
